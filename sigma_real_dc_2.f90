...
module L11_L12_exct_dc
    use prec
    use params
    use nabla, ONLY : fw,en,fw_2,en_2,psi_nabla_psi,psi_nabla_psi_2,wk,sind
    IMPLICIT NONE
! private variables
    ...
    real(q) :: pref,pref_2, dfermi,dfermi_2, pref1,pref2
    real(q) :: domega,sigma,T
    real(q) :: tmp1,tmp2,tmp3,tmp4,tmp5

    real(q) :: calc1, denerg, denerg_2
...
!
                do ibands=1,nbands  ! starting cycle over bands
                    do jbands=1,nbands   ! starting second cycle over bands
                          ...
                          if (sind==.false.) then ! the non-spin-polarized case:
        ...
        !
                                ...
                                * tmp2
                                ...
                                t22tmp(jcart,icart) = t22tmp(jcart,icart)&
                                    ...
                                    * tmp3 &
                                    * tmp4
                          else if (sind==.true.) then ! the spin-polarized case
                                if ( (.not.intrab) .and. (ibands==jbands) )  cycle
        !
                                if (.not.FROM_VASP) then
                                    denerg=(en(ibands,ikpoints) - en(jbands,ikpoints)) * 0.5d0
                                    ...
                                    denerg=(en(ibands,ikpoints) - en(jbands,ikpoints))
                                    denerg_2=(en_2(ibands,ikpoints) - en_2(jbands,ikpoints))
                                endif
        !
        !
                                if (dabs(denerg) <= de) then
                                    if (.not.intrab) cycle
                                    pref = 1.0_q / (dexp((en(jbands,ikpoints)-e_f)/dsigma)+1.d0)
                                    dfermi = -wk(ikpoints)*pref*(pref-1.d0)/dsigma*1.d0  ! 2.d0 facto is for ISPIN=1 and thus changed to 1.0
                                endif
                                if (dabs(denerg_2) <= de) then
                                    if (.not.intrab) cycle
                                    pref_2 = 1.0_q / (dexp((en_2(jbands,ikpoints)-e_f)/dsigma)+1.d0)
                                    dfermi_2 = -wk(ikpoints)*pref_2*(pref_2-1.d0)/dsigma*1.d0  ! 2.d0 facto is for ISPIN=1 and thus changed
                                endif
        !
                                if (dabs(denerg) > de) then
                                    if (.not.FROM_CODE) then  ! fermi-dirac formula to be used: e_f=mu, dsigma=Kb*T(K)
                                        pref1=1.0_q / (dexp((en(jbands,ikpoints)-e_f)/dsigma)+1.d0)
                                        pref2=1.0_q / (dexp((en(ibands,ikpoints)-e_f)/dsigma)+1.d0)
                                        dfermi = wk(ikpoints)*(pref1-pref2) / denerg*1.d0  !  2.d0 facto is for ISPIN=1 and thus changed
                                    else if (FROM_CODE) then  ! occupation output from the code: VASP or QE
                                        dfermi = (fw(jbands,ikpoints)-fw(ibands,ikpoints)) / denerg
                                    endif
                                endif
                                if (dabs(denerg_2) > de) then
                                    if (.not.FROM_CODE) then  ! fermi-dirac formula to be used: e_f=mu, dsigma=Kb*T(K)  NOT TESTED OR ALTERED
                                        pref1=1.0_q / (dexp((en_2(jbands,ikpoints)-e_f)/dsigma)+1.d0)
                                        pref2=1.0_q / (dexp((en_2(ibands,ikpoints)-e_f)/dsigma)+1.d0)
                                        dfermi_2 = wk(ikpoints)*(pref1-pref2) / denerg*1.d0  !  2.d0 facto is for ISPIN=1 and thus changed
                                    else if (FROM_CODE) then  ! occupation output from the code: VASP or QE
                                        dfermi_2 = (fw_2(jbands,ikpoints)-fw_2(ibands,ikpoints)) / denerg_2
                                    endif
                                endif
        ...
        !
                                ...
                                * tmp2
                                t11tmp(jcart,icart) = t11tmp(jcart,icart)&
                                    + tmp5
                                ! spin-dn
                                tmp1=real(conjg(psi_nabla_psi_2(icart,jbands,ibands,ikpoints)) &
                                *  psi_nabla_psi_2(jcart,jbands,ibands,ikpoints))
                                tmp2=dexp((-denerg_2*denerg_2)/(sigma*sigma))
                                tmp3=en_2(ibands,ikpoints)-e_f
                                tmp4=en_2(jbands,ikpoints)-e_f
                                tmp5=dfermi_2 &
                                * tmp1 &
                                * tmp2
                                t11tmp(jcart,icart) = t11tmp(jcart,icart)&
